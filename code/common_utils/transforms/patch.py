...
@register_transform('selected_region_fixed_size_patch_seq')
class SelectedRegionFixedSizePatchSeq(object):
    ...
    def __call__(self, data):
        ...
        seq_nb = data['res_nb'] + data['chain_nb'] * 1e6  # (L, 3)
        pos_sel = seq_nb[select_flag]  # (S, 3)
        dist_from_sel = torch.cdist(seq_nb[..., None], pos_sel[..., None]).min(dim=1)[0]  # (L, )
        patch_idx = torch.argsort(dist_from_sel)[:self.patch_size]
